...
def data_test(test_loader,model,folds):
...
def main():
    ...
    model = inceptionv4.inceptionv4(2, pretrained="imagenet")
    # model = model_v4.v4(num_classes=2)
    # model = get_net()
    ...
    train_data_list, val_data_list = train_test_split(all_data, test_size=val_ratio, random_state=666,
    ...
    for epoch in range(start_epoch,config.epochs):
        ...
        is_lowest_loss = valid_loss[0] < lowest_loss
        best_precision1 = max(valid_loss[1],best_precision1)
        # 新增最低loss
        lowest_loss = min(valid_loss[0], lowest_loss)
        try:
        ...
        },is_best, is_lowest_loss, fold)  # 新增 is_lowest_loss
