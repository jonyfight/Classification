...
def D_test(test_loader,model,folds):
...
def main():
    ...
    model = inceptionv4.inceptionv4(2)
    # model = model_v4.v4(num_classes=2)  # 采用v4测试
    # model = get_net()
    ...
    best_model = torch.load("checkpoints/best_model/%s/0/lowest_loss.pth.tar"%config.model_name)
    # best_model = torch.load("checkpoints/best_model/%s/0/lowest_loss.pth.tar" % config.model_name)
    model.load_state_dict(best_model["state_dict"])
    D_test(test_dataloader,model,fold)
