...
# 新增is_lowest_loss
def save_checkpoint(state, is_best, is_lowest_loss, fold):
    filename = "./checkpoints/resnet50/0/_checkpoint.pth.tar"
    # filename = config.weights + config.model_name + os.sep +str(fold) + os.sep + "_checkpoint.pth.tar"
    torch.save(state, filename)
    if is_best:
        message = "./checkpoints/best_model/resnet50/0/model_best.pth.tar"
    ...
    # 新增代码
    if is_lowest_loss:
        message = "./checkpoints/best_model/resnet50/0/lowest_loss.pth.tar"
        # message = config.best_models + config.model_name + os.sep + str(fold) + os.sep + 'model_best.pth.tar'
        print("Get Better top1 : %s saving weights to %s" % (state["lowest_loss"], message))
        with open("./logs/%s.txt" % config.model_name, "a") as f:
            print("Get Better top1 : %s saving weights to %s" % (state["lowest_loss"], message), file=f)
        shutil.copyfile(filename, message)
